...
abstract contract Queue is ERC721 {
    ...
    event VoteCasted(uint256 indexed tokenId, bool indexed wasGood, uint256 indexed voteInfluence);
    ...
    constructor(
        ...
        require(_maxRefund < _costPerSecond * _minTimeActive);
        queueURI = _queueURI;  
    ...
    function dequeue() external {
        ...
        if (block.timestamp >= activeReleasePeriodTimestamp + releasePeriodLength) {
            ...
            if (activeTokenCounter.current() > totalTokens) {
                ...
                queuedGasCosts = (totalTokens + 1 - activeTokenId) * maxRefund;
            }
    ...
    function voteOnToken(uint256 _tokenId, bool _wasGoodIdentifier) public {
        ...
        emit VoteCasted(_tokenId, _wasGoodIdentifier, voteInfluence);
